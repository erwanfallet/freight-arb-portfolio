"""Golden tests du modèle de voyage.

Le voyage de référence, utilisé dans presque tous les tests ci-dessous :
Panamax 66 kt sur Santos -> Qingdao, TCE 15 000 $/j, VLSFO 500 $/t, MGO 700 $/t,
paramètres par défaut (ballast 100 %, 12,5 nd en charge, 13,0 nd sur ballast, 6 j de port,
commission 3,75 %).
    ...
    conso ballast = 24 x (13,0/12,5)^3 = 24 x 1,124864 = 26,996736 t/j
    soutes mer    = 500 x (26 x 36,666667 + 26,996736 x 35,256410)
                  = 500 x 1 905,141333 = 952 570,67 $
    soutes port   = 700 x 3,0 x 6 = 12 600 $
    affrètement   = 15 000 x 77,923077 = 1 168 846,15 $
    coût total    = 1 168 846,15 + 952 570,67 + 12 600 + 180 000 = 2 314 016,82 $
    cargo payante = 66 000 x (1 - 0,0375) = 63 525 t
    fret          = 2 314 016,82 / 63 525 = 36,426868 $/t
"""
...
# ===========================================================================
# Le voyage de référence, terme à terme
# ===========================================================================
def test_laden_days():
...
def test_sea_bunker_cost_uses_cube_law_on_the_ballast_leg():
    # 24 t/j de base a 12,5 nd, mais le ballast tourne a 13,0 -> 24 x 1,124864 = 26,996736
    assert _reference().bunker_sea_usd == pytest.approx(952_570.666667, abs=1e-4)


def test_port_bunker_uses_mgo_not_vlsfo():
    # 700 (MGO) x 3 t/j x 6 j = 12 600, et surtout PAS 500 x 3 x 6 = 9 000
    assert _reference().bunker_port_usd == pytest.approx(12_600.0)


def test_payable_cargo_is_net_of_commission():
    # 66 000 x (1 - 0,0375) = 63 525 : la commission reduit la cargaison payante,
    # elle ne s'ajoute pas au cout
...
def test_the_ballast_disagreement_is_worth_16_dollars_a_tonne():
    """Le chiffre qui porte la page : ce que coûte le désaccord, sur ce voyage.

    Santos -> Qingdao est un aller simple de 11 000 nm ; facturer le retour à vide ou non
    fait presque doubler la durée du cycle. L'écart n'est pas un raffinement de modèle,
    il est du même ordre que l'arb lui-même.

    Vue du desk trading, ballast = 0 :
        total_days  = 36,666667 + 6                          = 42,666667
        soutes mer  = 500 x 26 x 36,666667                   = 476 666,67
        affretement = 15 000 x 42,666667                     = 640 000,00
        cout total  = 640 000 + 476 666,67 + 12 600 + 180 000 = 1 309 266,67
        fret        = 1 309 266,67 / 63 525                   = 20,610258 $/t

    Vue du departement fret, ballast = 1 : 36,426868 $/t (voir docstring du module).
    L'ecart vaut donc 15,816610 $/t.
    """
...
# ===========================================================================
# Vitesse et loi cubique (V-H2)
# ===========================================================================
def test_cube_law_on_consumption():
    # 26 t/j a 12,5 nd -> a 15 nd : 26 x (15/12,5)^3 = 26 x 1,728 = 44,928
...
def test_slowing_down_saves_bunkers_but_costs_days():
    """L'arbitrage vitesse/soutes, dans les deux sens — aucun des deux n'est gratuit."""
...
def test_zero_speed_is_rejected():
    with pytest.raises(VoyageError, match="vitesse"):
        PANAMAX.sea_consumption(0.0, laden=True)


# ===========================================================================
# Soutes (V-H3) et invariants (V-H5)
# ===========================================================================
def test_bunker_price_moves_the_rate_materially():
    """Le second désaccord : la date de valorisation des soutes.

    Sur un voyage de 78 jours, +30 % de VLSFO ajoute 4,50 $/t au fret — soit près de la
    largeur entière de la bande de décision marginale.
    """
...
def test_negative_bunker_price_is_rejected():
    with pytest.raises(VoyageError, match="soutes"):
        voyage_freight_usd_t(
...
def test_negative_tce_is_rejected_as_a_data_diagnostic():
    with pytest.raises(VoyageError, match="TCE négatif"):
        voyage_freight_usd_t(
...
def test_absurd_bunker_share_is_flagged():
    ...
    assert any("soutes pèsent" in w for w in warnings)


# ===========================================================================
# L'inversion — le repli du gate T1-1
# ===========================================================================
def test_implied_tce_round_trips():
    """Quand les routes Baltic en $/t existent mais pas le TCE, on remonte l'un depuis l'autre."""
...
def test_a_published_rate_below_costs_implies_a_negative_tce():
    """Un taux de route publié sous le coût de voyage implique un TCE négatif.

    Ce n'est pas absurde — ça arrive en marché déprimé — mais c'est le genre de print qui
    doit être vu, pas absorbé silencieusement dans un arb.
    """
    implied = implied_tce_from_freight(
...
def test_series_intersects_without_filling():
    ...
    mgo = pd.Series(700.0, index=index).iloc[:6]      # trois jours manquants a la fin
...
def test_panama_canal_dues_show_up_in_the_waterfall():
    ...
    # 420 000 $ de droits / 63 525 t = 6,61 $/t : loin d'etre un detail
    assert usgulf.waterfall["droits de canal"] == pytest.approx(6.611, abs=1e-2)
